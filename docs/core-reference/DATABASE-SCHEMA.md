...
- **Task Management**: Task (optional `cost: Decimal(12,2)` + `costPaid: Boolean` + `costPaidAt: DateTime?` — single value per task, clearing cost auto-resets paid; indexed on (costPaid, cost)), TaskAssignment (many-to-many task↔user, composite PK on (taskId, userId), cascades on either delete), TaskComment, TaskActivity, TaskAttachment, TaskDependency (directional blocks/blocked-by link; unique on (blockingTaskId, blockedTaskId); cascades on task delete)
