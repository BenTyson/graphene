...
- Task costs: optional `cost` (Decimal(12,2)) + `costPaid` flag + `costPaidAt` on the Task model. Single value per task — multi-cost would be a schema change. Server normalizes/serializes via `normalizeCost()` and `serializeTask()` in `server/routes/tasks.js`. Clearing cost force-resets `costPaid`. New endpoints: `GET /api/tasks/costs/summary` (aggregate, excludes ARCHIVED) and `PATCH /api/tasks/:id/cost-paid`. Activity log: `cost_set | cost_changed | cost_removed | cost_paid | cost_unpaid`. Tasks tab has a third `taskViewMode === 'costs'` view with stat tiles + filterable/groupable table. Group-by uses a flat-rows helper `getCostsRows()` — templates-of-templates wrapping `<tr>` don't work in Alpine x-for, must flatten to header/task row objects (same pattern as `getTaskListRows()`).
- Tasks tab views: `taskViewMode` is `kanban | list | calendar | costs`. Calendar view is purely client-side — no API/schema changes; consumes `getFilteredTasks()` so existing filters (priority, assignee, tag, institution, goal, overdue, search, archived) flow in unchanged. State: `calendarCursor` (Date, first of shown month) and `calendarSubMode` (`month | agenda`, persisted to localStorage). Helpers in `app-refactored.js`: `getCalendarWeeks()` (6 weeks Sun→Sat with `{ date, ymd, isCurrentMonth, isToday, isPast, tasks }`), `getCalendarAgenda()` (Overdue group + 60-day forward window grouped by ymd), `getTasksByDueDate()` (Map ymd → tasks, priority-sorted), `getCalendarPillClass(task)` (priority-tinted with red override for overdue, line-through for DONE/ARCHIVED), `openTaskFormForDate(ymd)`. Month grid is desktop-only (`md:` hidden on mobile); agenda is always shown on mobile and optionally on desktop. Top-level tasks only — subtask due dates and goal `targetDate` are deferred. ymd buckets use a local-time `_ymd()` helper, not `toISOString()` (which would shift dates in non-UTC timezones).
...
- Proforma editor sections are top-level "journey pills" in `client/src/js/components/tabs/proforma/cards.js` → `JOURNEY_SECTIONS`. Order: Revenue → Markets → Production → Costs → Operations → Machines → Capital. Each pill flips `proformaSection` and renders one section component. Battery/supercap physical specs live in a "Reference data" accordion that only shows on the Markets section (collapsed by default) — they feed the built-in market sources and are rarely edited. Section bodies use plain `card()` stacks; the older `criticalBlock`/`advancedAccordion` helpers in `helpers.js` still exist but are unused — don't reach for them, expose content directly in named cards instead.
...
- Proforma revenue streams: every scenario ships with three built-in streams (`supercapElectrode`, `carbonBlackCathodeAnode`, `grapheneOxideStream`) each pre-linked to a market source. Built-ins can be edited but not deleted. Every stream has an `enabled: boolean` (default true) — `computeRevenue()` skips disabled streams but still emits a zero-stripe so chart datasets stay stable in size/color. Toggle via `toggleProformaRevenueStream(id)` (header switch on the Revenue tab card).
- Proforma Direct-$ streams have a sub-toggle for which side drives the math: `market.directInput: 'revenue' | 'kg'` (default `'revenue'`). `'revenue'` mode keeps `market.revenueByYear` as the input and derives kg = revenue ÷ price. `'kg'` mode reads `market.kgByYear` as the input and derives revenue = kg × price. Both fields persist at all times so toggling doesn't lose entries. `migrateAssumptions()` seeds both onto legacy direct streams. The Revenue editor surfaces a $/kg price-per-year row in both modes plus a derived-side chip (`≈ N kg` or `≈ $N`) under each yearly input; falls back to `⚠ set price below` when the input is set but price is 0.
- Proforma kg series: `computeRevenue()` emits `kgByStream`/`kgTotal` alongside dollar arrays. Linked-mode = `marketKg × marketSharePct`. Direct-mode branches on `market.directInput`: `'revenue'` → `revenue / pricePerKg` (0 if no price); `'kg'` → reads `kgByYear` directly, revenue = kg × price. `assembleOutlook()` exposes `productionCapacityKg` (= `production.monthlyGrapheneKg`), `demandKgTotal`, `capacityShortfallKg`, and `demandKg_<streamId>` per stream — all participate in `aggregateViews()` so quarterly/yearly views populate automatically. Surfaced as the "Production Capacity vs Sales Demand" chart on the Charts sub-tab and the per-year strip at the top of the Machines section.
- Proforma Production sub-tab (`ProductionTimeline.js`) is a single combined table: Machine Timeline (gantt — phase blocks slate=construction/amber=validation/emerald=production + capex payment pills overlaid) + Production Detail (machines, totals, hemp inputs, costs, INVENTORY). Both halves share one `<table>` with the same 60-monthly + 5-yearly-total column structure; gantt yearly totals show per-machine yearly capex. Inventory rows derived in `getProductionTableRows()` from `outlook.demandKgTotal` (kg sold) and `production.monthlyGrapheneKg` (produced) — Ending = beginningInventory + produced − sold, can go negative (shown red) when planned sales exceed capacity. Yearly Available = carry-in + produced-in-year; Yearly Ending = December's value. State: `proformaProductionCollapsed` (per-section collapse).
- Proforma table section color palette: parent rows use a Tailwind -100 shade, totals a -200, children a /25-opacity -50 for very faint banding. Eight sections tagged on Outlook rows via `row.section`: `sales`/`cogs`/`margin`/`opex`/`ebitda`/`capex`/`capital`/`cash`. Class strings live in `ProformaService.js` `SECTION_TINTS` as literal Tailwind classes (JIT requirement). Children's sticky label cells use `bg-white` not the transparent tint — semi-transparent backgrounds on sticky cells let horizontally-scrolled content bleed through. The same palette applies to Summary's yearly breakdown table.
- Proforma table sticky thead: Outlook + Production tables use `<thead class="sticky top-0 z-20">` so year/month rows stay visible during vertical scroll. Wrappers use `overflow-auto max-h-[calc(100vh-N)]` (sticky needs a constrained scrolling ancestor). Leftmost sticky-left `<th>` cells in the thead need z-30 to layer above their thead row.
- Proforma Summary tab (`ProformaTab.js` `_summaryView()`): headline striped P&L table at top (Revenue / COGS / Gross / Gross Margin % / OPEX / Net Income Pre-Tax / Net Operating Margin %; label column always one shade darker than its row's alternating data column). Yearly breakdown table below uses the 8-section color palette. Cash Needs tiles below it (peak deficit + CapEx-through-break-even + OpEx-through-break-even + break-even month). Assembled by `getSummary()` in `ProformaService.js`. Net Income Pre-Tax = `yearly.ebitda` (no separate tax model). "Total Cash to Break Even" = `|peakCashNeed|` (net deficit including revenue/capital/openingCash offsets) and does NOT equal CapEx + OpEx to-break-even (those are gross outflows).
- Email digests (Sparrow → Listmonk → Postmark): three transactional templates — `weeklyDigest`, `dueTomorrow`, `overdue`. HTML source lives in `shared/emailTemplates.js` (diffable in git); the running templates are pasted into Listmonk admin and referenced by integer `template_id` stored on `EmailSettings`. Cron endpoints under `server/routes/emailCron.js` are gated by `EMAIL_CRON_SECRET` (middleware in `server/middleware/cronAuth.js`). Payload shapes are produced by `server/services/emailDigest.js` — `buildWeeklyDigest` returns `{ overdue, due_this_week, recently_done, goal_progress, upcoming_due_soon_count, total_count, week_label, user_name }` and skips when both overdue + due_this_week are empty. Subject lines for due-tomorrow/overdue use a `count==1` branch with a `title` field; the digest pre-computes `title` only when count===1. Templates use a tiny Go-template subset (`{{ .path }}`, `{{ if/else/end }}`, `{{ range }}`, `{{ eq }}`); never include `{{ template "content" . }}` (campaign-only, breaks tx). Preview locally with `node scripts/preview-email.mjs --type weekly|due-tomorrow|overdue [--user <id> | --email <addr> | --mock]`. After editing a template in Listmonk admin, copy the final HTML back into `shared/emailTemplates.js` to keep source-of-truth in sync. See `docs/EMAIL-TEMPLATES.md` for the paste-into-Listmonk workflow.
...
- Proforma operating schedule is per-kiln-type: `production.phasesByMachineType: { pilot: [×3], broderick: [×3] }`. The old single `production.phases[]` is gone — `migrateAssumptions()` deep-clones the legacy array into both buckets and deletes the old field (idempotent). Both production rate AND op cost (labor + maintenance) follow the matching type's schedule in `computeProduction()`; `baseCostByPhase` is now `{ pilot, broderick }`. Hemp ratio is per-hour math, unaffected. Machine-level `productionPhaseOverride`/`costPhaseOverride` are still phase indices 0/1/2 — they index into the machine type's own schedule. ProductionSection renders two stacked sub-cards with a "Copy from pilot" shortcut on the broderick header.
- Proforma investor sharing (`docs/builds/investor-proforma-sharing.md`): a master scenario can be shared as an isolated **variant clone** (`isVariant:true`, `parentId`) reached only by a share token, never a graphene login. Admins mint/copy/revoke links from the **Share modal** (link icon on each proforma scenario card → `_shareModal()` in `ProformaTab.js`; pick View-only/Editable, auto-copies the URL, lists + revokes active links). It calls `POST /api/proforma/scenarios/:id/share` (returns `{ share, variant, embedUrl }`), `GET /scenarios/:id/shares`, and `POST /shares/:shareId/revoke`; URLs are rebuilt client-side as `${location.origin}/proforma-embed.html?token=`. The modal is `<template x-if="proformaShareModal">`-guarded + lives only in list view, so it never leaks into the embed page. the **token router** `server/routes/proformaShare.js` (mounted at `/api/proforma/share` BEFORE `/api/proforma`, NO `requireSuperAdmin`) serves `GET/PUT /share/:token` and authenticates by token only — no scenario id is ever accepted from the client, so a master is unreachable (master-safety invariant). PUT is edit-mode-only; view-PUT/master-pointing/revoked → 403/404. **The global `/api` write-guard in `server/index.js` skips `/proforma/share`** so the token PUT reaches its own auth (it JWT-guards every other POST/PUT/DELETE). The chrome-less embed page is a **separate Vite entry**: `client/proforma-embed.html` + `client/src/js/proforma-embed.js` (`window.proformaEmbedApp`) — a SLIM Alpine factory (proforma state + delegates only, NOT `grapheneApp()`, whose `init()` fires auth loaders that error without a JWT). It reads `?token=`, GETs the variant, mounts `getProformaTabHtml()`; view mode renders via the editor's existing `locked` path, edit mode Saves by PUTing `{ assumptions }` to the token. In the Assumptions sub-tab the journey-pill nav (`metricsAndNav()`) sits OUTSIDE the lock wrapper, so a locked/view-only viewer can still click through sections to read the data — only the input content below carries `pointer-events-none`. `vite.config.js` uses `build.rollupOptions.input` for multi-entry; build emits both `index.html` and `proforma-embed.html`. Phase 3 (the hgdeck-side deck link/toggle) lives in the sibling `hgdeck` repo.

- Test Matrix (Tests section, `test-matrix` tab): a code-defined matrix of which characterization/QC tests each application (industry + use-case) requires. Data source-of-truth is `client/src/js/data/testMatrix.js` (NOT DB-backed) — exports `TEST_MATRIX_TESTS` (columns, grouped via `group`), `TEST_MATRIX_APPLICATIONS` (rows: `{ industry, application, material, notes, req }` where `req` maps `testId -> { level, target?, note? }`), `TEST_MATRIX_LEVELS` (`required|recommended|optional`, each carrying literal Tailwind `cellClass`/`dotClass` for JIT), plus derived `*_BY_ID`/`TEST_MATRIX_TEST_GROUPS` lookups. All are mirrored onto `window.*` so the tab template + Alpine helpers read them without imports. Omitting a `testId` from `req` = "not evaluated". Tab component is `client/src/js/components/tabs/TestMatrixTab.js` → `getTestMatrixTabHtml()` (desktop = sticky-first-column table with grouped header band + per-row required/total badge; mobile = cards). Alpine state: `testMatrixSearch`/`testMatrixIndustryFilter`/`testMatrixMaterialFilter`; helpers (`getTestMatrix*`) live in app-refactored.js near `isTestResultsTab()`. To add data just edit the data file (add a column, add a row, or fill a `req` cell) — no schema/API changes. Trivially convertible to a Prisma model later if in-app editing is ever needed.
